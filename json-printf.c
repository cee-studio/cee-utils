...
#define ASSIGN_IF_NOT_ZERO(lhs, exp)                                          \
  if (lhs) {                                                                  \
    lhs = exp;                                                                \
  }
  ...
  for (i = 0; i < number_of_specifiers; i++) {
    ...
    if (cur_ptr) memcpy(cur_ptr, format + sp[i].start, slen);
    /*cur_ptr += slen; */
  ...
  if (number_of_specifiers) {
    slen =
      snprintf(cur_ptr, len, "%s", format + sp[i - 1].after_specifier_pos);
    total_size += slen;
  }
  else {
    if (cur_ptr) strcpy(cur_ptr, format);
