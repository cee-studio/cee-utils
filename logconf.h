...
/**
 * @brief A stackful and modularized wrapper over the popular 'log.c' facilities
 *
 * Provides a way to split logging into individual modules, that can be configured
 *        separately from one another, or have their resources shared via branching.
 */
struct logconf {
  /** logging module id */
 ...
 * @param fp the configuration file pointer that will fill `struct logconf` fields
 */
 ...
 *
 * Initialize a `branch` logging module thats expected to share common resources with 
 *        its parent parent module `orig`. The common resources include: config file
 *        directives, logging output and disabled modules list.
 * @param branch pointer to the `struct logconf` structure to be initialized as `orig` branch
 * @param orig pointer to the `struct logconf` structure that holds the parent module
